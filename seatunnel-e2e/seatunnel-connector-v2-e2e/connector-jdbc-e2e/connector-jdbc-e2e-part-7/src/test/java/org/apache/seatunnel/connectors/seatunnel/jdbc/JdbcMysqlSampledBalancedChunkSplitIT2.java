...
public class JdbcMysqlSampledBalancedChunkSplitIT2 extends TestSuiteBase implements TestResource {
    private static final Logger LOG = LoggerFactory.getLogger(JdbcMysqlSampledBalancedChunkSplitIT2.class);
    ...
    private static final int MYSQL_PORT = 3314;
    private static final int BUCKET_NUM = 6;
    ...
    public void testSplit() throws Exception {
        ...
        configMap.put("sampling_percentage", 1);  // 测试数据量太少以至于不建议采样
        configMap.put("bucket_number", BUCKET_NUM);
        ...
        Assertions.assertEquals(BUCKET_NUM, jdbcSourceSplits.size());
...
//        assertNumSplit(splitArray, "");
        ...
        splitArray = getCheckedSplitArray(configMap, table, "c_tinyint", BUCKET_NUM);
//        assertNumSplit(splitArray, "");

        // use tinyint_unsigned column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_tinyint_unsigned", BUCKET_NUM);
        configMap.put("partition_column", "c_tinyint_unsigned");

        // use smallint column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_smallint", BUCKET_NUM);
        configMap.put("partition_column", "c_smallint");

        // use smallint_unsigned column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_smallint_unsigned", BUCKET_NUM);
        configMap.put("partition_column", "c_smallint_unsigned");

        // use int column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_int", BUCKET_NUM);
        configMap.put("partition_column", "c_int");

        // use int column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_integer", BUCKET_NUM);
        configMap.put("partition_column", "c_integer");

        // use int_unsigned column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_int_unsigned", BUCKET_NUM);
        configMap.put("partition_column", "c_int_unsigned");

        // use integer_unsigned column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_integer_unsigned", BUCKET_NUM);
        configMap.put("partition_column", "c_integer_unsigned");

        // use int column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_mediumint", BUCKET_NUM);
        configMap.put("partition_column", "c_mediumint");

        // use int column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_mediumint_unsigned", BUCKET_NUM);
        configMap.put("partition_column", "c_mediumint_unsigned");

        // use bigint column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_bigint", BUCKET_NUM);
        configMap.put("partition_column", "c_bigint");

        // use bigint_unsigned column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_bigint_unsigned", BUCKET_NUM);
        configMap.put("partition_column", "c_bigint_unsigned");

        // use decimal column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_decimal", BUCKET_NUM);
        configMap.put("partition_column", "c_decimal");

        // use decimal_unsigned column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_decimal_unsigned", BUCKET_NUM);
        configMap.put("partition_column", "c_decimal_unsigned");

        // use double column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_double", BUCKET_NUM);
        configMap.put("partition_column", "c_double");

        // use unsigned double column to split
        splitArray = getCheckedSplitArray(configMap, table, "c_double_unsigned", BUCKET_NUM);
        configMap.put("partition_column", "c_double_unsigned");

        // use date column to split
        configMap.put("partition_column", "c_date");
        splitArray = getCheckedSplitArray(configMap, table, "c_date", BUCKET_NUM);
        configMap.put("partition_column", "c_date");
